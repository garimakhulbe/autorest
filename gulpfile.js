...
gulp.task('regenerate:delete', function(cb){
  del([
    ...
    'src/generator/AutoRest.Go.Tests/src/generated'
  ], 
  cb);
});
...
function testGo() {
  process.env.GOPATH = basePathOrThrow() + '/src/generator/AutoRest.Go.Tests';
  var result = shell.task(['glide up', 'go run ./runner.go'], {
      cwd: process.env.GOPATH + './src', verbosity: 3});
  return result
}
