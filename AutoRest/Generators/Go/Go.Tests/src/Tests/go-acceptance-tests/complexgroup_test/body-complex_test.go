...
func (s *ComplexGroupSuite) TestGetValidReadOnlyComplex(c *chk.C) {
	res, err := complexReadOnlyClient.GetValid()
	c.Assert(err, chk.IsNil)
	_, err = complexReadOnlyClient.PutValid(res)
	c.Assert(err, chk.IsNil)

	c.Assert(*res.ID, chk.Equals, "1234")
	c.Assert(*res.Size, chk.Equals, 2)
}

func (s *ComplexGroupSuite) TestPutValidReadOnlyComplex(c *chk.C) {
	id, size := "1234", int32(2)
	arg := ReadonlyObj{
		ID:   &id,
		Size: &size,
	}
	_, err := complexReadOnlyClient.PutValid(arg)
	c.Assert(err, chk.IsNil)
}
