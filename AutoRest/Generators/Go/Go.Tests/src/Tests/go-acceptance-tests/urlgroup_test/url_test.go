...
func (s *URLSuite) TestPathByteNull(c *chk.C) {
	_, err := pathClient.ByteNull(nil)
	c.Assert(err, chk.IsNil)
}

func (s *URLSuite) TestPathDateNull(c *chk.C) {
	_, err := pathClient.DateNull(date.Date{})
	c.Assert(err, chk.NotNil)
}

func (s *URLSuite) TestPathDateTimeNull(c *chk.C) {
	_, err := pathClient.DateTimeNull(date.Time{})
	c.Assert(err, chk.NotNil)
}
